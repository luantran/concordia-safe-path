...
Before designing any screens, the focus was on mapping out exactly how each user would move through the app step by step. These diagrams show every decision point and every action the user or the system takes along the way. Each flow is tied to one of the personas so that the logic reflects what Sarah, Chloe, and James actually need to do, rather than what was assumed.
 
To illustrate how users move through the system, four user flows were developed, each targeting a distinct interaction context.
 
\subsubsection{Login / Sign Up}
The first covers account creation and login, following the steps a new user takes to register, select their role, and configure their accessibility and notification preferences. This flow is particularly relevant to Sarah and Chloe, whose safety and comfort depend heavily on the system being correctly configured to their needs from the outset.
 
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth, keepaspectratio]{user_flows/Login_Signup_low.png}
  \caption{Login / Sign Up user flow.}
  \label{fig:flow_login}
\end{figure}
 
\subsubsection{Main Navigation}
The second covers main navigation, showing how a user moves between the core sections of the interface during a typical campus disruption. This is Chloe's primary mode of use: passively monitoring the incident map and checking report status.
 
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth, keepaspectratio]{user_flows/CoreUI_low.png}
  \caption{UI navigation user flow.}
  \label{fig:flow_navigation}
\end{figure}
 
\subsubsection{Menu}
The third covers accessing the menu, following how users reach emergency contacts, accessibility settings, and notification preferences.
 
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth, keepaspectratio]{user_flows/Menu_low.png}
  \caption{Menu user flow.}
  \label{fig:flow_menu}
\end{figure}
 
\subsubsection{Route Planning}
The fourth covers route planning, following the steps a user takes to enter a destination, review safe and accessible route options filtered by their accessibility needs, and begin navigation. This is Sarah's most critical flow, as it directly addresses her need to verify that a route is wheelchair-accessible before leaving her starting point.
 
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth, keepaspectratio]{user_flows/RoutePlanning_low.png}
  \caption{Route planning user flow.}
  \label{fig:flow_route}
\end{figure}
 
Together these four flows span the full range of typical interactions and map directly onto the goal categories defined in Section~\ref{sec:goals}: setup and personalization, situational awareness, navigation, and emergency and resource access.
...
The screens presented in this section are screenshots taken directly from the functional Android application, representing the end result of the design process described in Section~\ref{sec:methodology}. The high-fidelity Figma prototypes served as the direct reference for implementation: layout, color system, typography, and interaction patterns were carried through from design into code with minimal deviation. Where implementation constraints required adjustments, the underlying design rationale --- calm technology, WCAG AA compliance, and reduced cognitive load --- remained the guiding principle.
 
Concordia Safe Path is implemented as a cross-platform mobile application targeting Android, built with React Native and Expo Router. Expo Router provides a file-based navigation architecture suited to the multi-screen structure of the system. The backend is powered by Supabase, which provides real-time database capabilities, user authentication, and storage built on PostgreSQL. Row Level Security (RLS) policies enforce role-based access control at the database level, ensuring that staff-only actions such as incident verification cannot be performed by student accounts at the API level. Campus map rendering, routing, and incident visualization are handled through the Google Maps SDK via the \texttt{react-native-maps} library. Production builds are distributed as APKs via EAS Build.
 
Development followed standard software engineering practices throughout. The codebase is hosted on GitHub at \url{https://github.com/luantran/concordia-safe-path}, with all feature work carried out on dedicated branches and integrated into the main branch exclusively through pull requests. Each pull request required a written description, screenshots of affected screens, and at least one team member review before merging. A shared pull request template enforced consistency across contributions. Work was tracked using GitHub Projects with a Kanban board, allowing the team to coordinate tasks across the six contributors and maintain visibility into what was in progress, under review, and complete. This workflow ensured that the codebase remained stable and reviewable at every stage of development.
